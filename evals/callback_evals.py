...
async def test_agent_callbacks():
    ...
    assert 'processing_start_time' in context.state, "Should track processing start time"
    ...
    assert 'last_processing_duration' in context.state, "Should track processing duration"
...
async def test_model_callbacks():
    ...
    response = MockLlmResponse("Test model response", {
        'total_tokens': 150,
        'prompt_tokens': 75,
    ...
    assert 'model_request_start_time' in context.state, "Should track model request start time"
...
async def test_tool_callbacks():
    ...
    assert context.state[f'{tool.name}_last_duration'] > 0, "Tool duration should be positive"
...
async def test_callback_integration():
    ...
    assert root_agent.before_agent_callback is not None, "Agent should have before_agent_callback"
    assert root_agent.after_agent_callback is not None, "Agent should have after_agent_callback"
    assert root_agent.before_model_callback is not None, "Agent should have before_model_callback"
    assert root_agent.after_model_callback is not None, "Agent should have after_model_callback"
    assert root_agent.before_tool_callback is not None, "Agent should have before_tool_callback"
    assert root_agent.after_tool_callback is not None, "Agent should have after_tool_callback"
    
    # Test that callbacks are the correct functions
    assert root_agent.before_agent_callback == before_agent_callback, "Should use our before_agent_callback"
    assert root_agent.after_agent_callback == after_agent_callback, "Should use our after_agent_callback"
...
async def test_rag_memory_integration():
    ...
    try:
        ...
        try:
            memories = await retrieve_user_memories("rag_test_user", "test memory query")
...
async def test_performance_monitoring():
    ...
    assert len(performance_data) == 3, "Should have collected performance data for all interactions"
    
    # Calculate average performance
    avg_duration = sum(p['processing_duration'] for p in performance_data) / len(performance_data)
