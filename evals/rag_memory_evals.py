...
from sim_guide.rag_memory_service import (
    create_corpus,
    list_corpora,
    get_corpus,
    delete_corpus,
...
class RagMemoryEvals:
    ...
    async def eval_corpus_querying(self) -> Dict[str, Any]:
        ...
        try:
            ...
            corpus_response = await create_corpus(
                display_name=corpus_name,
                description="Test corpus for query evaluation"
            )
            ...
            results["passed"] = len(query_results) > 0 and search_response["status"] == "success"
            results["details"] = {
                "corpus_id": corpus_id,
                "query_results": query_results,
                "search_all_response": search_response,
                "successful_queries": len([q for q in query_results if q["has_results"]])
            }
    ...
    async def eval_memory_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["passed"] = (
                memory_response["status"] == "success" and
                len(retrieval_results) > 0 and
                empty_retrieval["status"] == "success" and
                len(empty_retrieval["memories"]) == 0
            )
            ...
            results["details"] = {
                ...
                "successful_retrievals": len([r for r in retrieval_results if r["has_memories"]]),
