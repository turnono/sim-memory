...
RAG_COST_OPTIMIZED = os.getenv("RAG_COST_OPTIMIZED", "false").lower() == "true"
MAX_CORPORA_SEARCH = int(os.getenv("MAX_CORPORA_SEARCH", "3"))  # Limit corpus searches

# Use existing staging bucket instead of creating new ones
RAG_STAGING_BUCKET = os.getenv(
...
async def create_corpus(
    ...
    try:
        ...
        rag_corpus = rag.create_corpus(
            ...
            or f"RAG corpus for simulation guidance: {display_name}",
...
async def list_corpora() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "corpora": corpus_list, "count": len(corpus_list)}

    except Exception as e:
        logger.error(f"Failed to list RAG corpora: {e}")
        return {"status": "error", "error_message": str(e), "corpora": []}
...
async def get_corpus(corpus_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "error_message": str(e), "corpus_id": corpus_id}
...
async def upload_document_to_gcs(
    file_content: bytes, file_name: str, content_type: Optional[str] = None
) -> Dict[str, Any]:
    """
...
async def import_document_to_corpus(
    corpus_id: str, gcs_uri: str, display_name: Optional[str] = None
) -> Dict[str, Any]:
    """
...
async def search_all_corpora(
    query: str, top_k_per_corpus: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # COST OPTIMIZATION: Limit number of corpora searched
        corpora_to_search = corpora_response["corpora"]
        if RAG_COST_OPTIMIZED:
            # In cost-optimized mode, limit to MAX_CORPORA_SEARCH most recent corpora
            corpora_to_search = corpora_to_search[:MAX_CORPORA_SEARCH]
            logger.info(
                f"Cost optimization: limiting search to {len(corpora_to_search)} corpora"
            )

        for corpus_info in corpora_to_search:
            corpus_id = corpus_info["corpus_id"]

            # Query each corpus
            corpus_results = await query_corpus(
                corpus_id=corpus_id, query=query, top_k=top_k_per_corpus
            )
        ...
        search_type = "cost-optimized" if RAG_COST_OPTIMIZED else "full"
        logger.info(
            f"Search across corpora ({search_type}) returned {len(all_results)} total results"
        )
        ...
        return {
            ...
            "searched_corpora_count": len(corpora_to_search),
            "total_corpora_count": len(corpora_response["corpora"]),
            "cost_optimized": RAG_COST_OPTIMIZED,
        }
...
async def add_memory_from_conversation(
    ...
    """
    try:
        # COST OPTIMIZATION: Skip expensive embedding operations in cost-optimized mode
        if RAG_COST_OPTIMIZED:
            logger.info(
                f"Cost optimization: skipping memory storage for user {user_id}"
            )
            return {
                "status": "success",
                "message": "Memory storage skipped (cost optimization enabled)",
                "user_id": user_id,
                "session_id": session_id,
                "corpus_id": "cost-optimized-skip",
        ...
        memory_doc = {
            ...
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "metadata": {"source": "sim_guide_session", "version": "1.0"},
        }
...
"""
        ...
        # Upload to GCS
        filename = (
            f"memory_{user_id}_{session_id}_{int(datetime.now().timestamp())}.txt"
        )
...
async def retrieve_user_memories(
    user_id: str, query: str, top_k: Optional[int] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.debug(
            f"Retrieved {len(query_response['results'])} memories for user {user_id}"
        )
