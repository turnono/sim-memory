...
from .user_models import UserPreferences, LifeExperienceLevel, CommunicationStyle, LifeArea
...
class UserPreferenceDetector:
    ...
    def __init__(self):
        ...
        self.life_area_patterns = {
            ...
            LifeArea.RELATIONSHIPS: [r'\b(relationship|dating|marriage|family|friends)\b'],
            ...
            LifeArea.PERSONAL_GROWTH: [r'\b(personal growth|self improvement|learning|skills)\b'],
            LifeArea.PRODUCTIVITY: [r'\b(productivity|time management|organization|efficiency)\b'],
            ...
            LifeArea.SPIRITUALITY: [r'\b(spiritual|meaning|purpose|meditation|mindfulness)\b'],
    ...
    def detect_preferences_from_text(self, text: str, current_prefs: UserPreferences) -> UserPreferences:
        ...
        if experience_scores:
            ...
            if confidence > updated_prefs.preference_confidence.get('life_experience_level', 0):
                updated_prefs.life_experience_level = detected_level
                updated_prefs.preference_confidence['life_experience_level'] = confidence
                logger.info(f"Detected life experience level: {detected_level.value} (confidence: {confidence:.2f})")
        ...
        if communication_scores:
            ...
            if confidence > updated_prefs.preference_confidence.get('communication_style', 0):
                updated_prefs.communication_style = detected_style
                updated_prefs.preference_confidence['communication_style'] = confidence
                logger.info(f"Detected communication style: {detected_style.value} (confidence: {confidence:.2f})")
...
def update_user_preferences(session_state: Dict[str, Any], preferences: UserPreferences) -> None:
    ...
    logger.info(f"Updated life guidance preferences for user: {preferences.user_name or 'unknown'}")


def analyze_user_message_for_preferences(message: str, session_state: Dict[str, Any]) -> UserPreferences:
...
def format_preferences_summary(preferences: UserPreferences) -> str:
    ...
    summary_parts.append(f"Life Experience: {preferences.life_experience_level.value.replace('_', ' ').title()}")
    summary_parts.append(f"Communication Style: {preferences.communication_style.value.replace('_', ' ').title()}")
    
    if preferences.focus_life_areas:
        life_areas = [area.value.replace('_', ' ').title() for area in preferences.focus_life_areas]
        summary_parts.append(f"Life Focus Areas: {', '.join(life_areas)}")
    
    if preferences.current_life_goals:
        summary_parts.append(f"Current Goals: {', '.join(preferences.current_life_goals)}")
...
def get_personalized_instruction_context(preferences: UserPreferences) -> str:
    ...
    if preferences.life_experience_level == LifeExperienceLevel.YOUNG:
        context_parts.append("User is young and starting their life journey - provide foundational guidance and encouragement.")
    elif preferences.life_experience_level == LifeExperienceLevel.EXPERIENCED:
        context_parts.append("User is experienced in life - provide sophisticated insights and advanced strategies.")
    elif preferences.life_experience_level == LifeExperienceLevel.MATURE:
        context_parts.append("User has mature life wisdom - engage with respect and offer perspective-based guidance.")
    ...
    if preferences.communication_style == CommunicationStyle.CONCISE:
        ...
        context_parts.append("User prefers step-by-step life guidance and action plans.")
    elif preferences.communication_style == CommunicationStyle.MOTIVATIONAL:
        context_parts.append("User responds well to encouraging and inspirational guidance.")
    ...
    if preferences.focus_life_areas:
        life_areas = [area.value.replace('_', ' ') for area in preferences.focus_life_areas]
        context_parts.append(f"User is focused on these life areas: {', '.join(life_areas)}.")
    
    if preferences.current_life_goals:
        context_parts.append(f"User's current goals: {', '.join(preferences.current_life_goals)}.")
