...
from ..services import (
    UserPreferences, LifeExperienceLevel, CommunicationStyle, LifeArea,
    get_user_preferences as service_get_user_preferences,
    update_user_preferences as service_update_user_preferences, 
    format_preferences_summary, get_personalized_instruction_context,
...
def set_user_preference(preference_name: str, preference_value: str) -> str:
    ...
    try:
        ...
        if preference_name == "name" or preference_name == "user_name":
        ...
        elif preference_name == "life_experience_level" or preference_name == "experience_level":
            try:
                preferences.life_experience_level = LifeExperienceLevel(preference_value.lower())
            except ValueError:
            ...
            try:
                preferences.communication_style = CommunicationStyle(preference_value.lower())
            except ValueError:
...
def analyze_message_for_preferences(message: str) -> str:
    ...
    try:
        ...
        updated_prefs = service_analyze_user_message_for_preferences(message, session_state)
        ...
        if updated_prefs.life_experience_level != original_prefs.life_experience_level:
            changes.append(f"Life Experience: {updated_prefs.life_experience_level.value}")
            
        if updated_prefs.communication_style != original_prefs.communication_style:
            changes.append(f"Communication Style: {updated_prefs.communication_style.value}")
            
        if len(updated_prefs.focus_life_areas) > len(original_prefs.focus_life_areas):
            new_areas = set(updated_prefs.focus_life_areas) - set(original_prefs.focus_life_areas)
        ...
        if len(updated_prefs.preferred_tools) > len(original_prefs.preferred_tools):
            new_tools = set(updated_prefs.preferred_tools) - set(original_prefs.preferred_tools)
            changes.append(f"New Tools: {list(new_tools)}")
            
        if len(updated_prefs.current_life_goals) > len(original_prefs.current_life_goals):
            new_goals = set(updated_prefs.current_life_goals) - set(original_prefs.current_life_goals)
...
def get_personalization_context() -> str:
    ...
    try:
        ...
        return personalization_context if personalization_context else "No specific personalization context available - user preferences not yet established"
