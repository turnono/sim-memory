...
def analyze_session_context() -> str:
    ...
    try:
        ...
        if loaded_resources:
            context_info.append(
                f"Resources available: {', '.join(loaded_resources[:3])}"
            )
        ...
        if preloaded_contexts:
            context_info.append(
                f"Preloaded contexts: {', '.join(preloaded_contexts[:2])}"
            )
...
def get_conversation_continuity_hints() -> str:
    ...
    try:
        ...
        if resources:
            hints.append(
                f"Available resources to reference: {', '.join(resources[:2])}"
            )

        if not hints:
            hints.append(
                "Well-established session - maintain current conversation flow"
            )
